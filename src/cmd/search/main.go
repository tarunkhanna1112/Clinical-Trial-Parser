...
func (m *Matcher) batchSearch() {
	...
	// To map it to our own ontologies
	header := "# Autogenerated from UMLS\tterm\tmesh_concepts\n"
	outfname := m.parameters.Get("output_file")
	writer := fio.Writer(outfname)
	defer writer.Close()
	writer.WriteString(header)

	nodes := csv.NewReader(f)
	nodes.Comment = rune(param.Comment)
	count := 1
	for {
		line, err := nodes.Read()
		if err == io.EOF {
		...
		matches := m.vocabulary.Match(line[1], matchMargin, emptyCategories)
		fmt.Printf(line[0] + "\n")

		writer.WriteString(line[0] + "\t" + line[1] + "\t")
		writer.WriteString(strings.ReplaceAll(matches.String(), "\n", ";") + "\n")
